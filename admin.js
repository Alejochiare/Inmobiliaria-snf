...
/* ─── SHARED DATA ─── */
const DEFAULT_PROPS = [
  { id:1, title:'Casa amplia a metros del centro', price:'$48.000.000', op:'venta', loc:'Balnearia, Córdoba', beds:3, baths:2, m2:'180', img:'https://images.unsplash.com/photo-1568605114967-8130f3a36994?w=700&q=80', desc:'Hermosa propiedad en excelentes condiciones. Cuenta con jardín delantero y trasero, cochera cubierta para 2 autos y amplio patio. Ideal para familia.', status:'active' },
  ...
  { id:3, title:'Terreno esquina — Ideal inversión', price:'$12.000.000', op:'terreno', loc:'Balnearia Norte', beds:0, baths:0, m2:'450', img:'https://images.unsplash.com/photo-1500382017468-9049fed747ef?w=700&q=80', desc:'Excelente terreno en esquina ideal para proyecto residencial o comercial. Todos los servicios disponibles.', status:'active' },
  { id:4, title:'Casa familiar con pileta y parque', price:'$62.000.000', op:'venta', loc:'Balnearia, Córdoba', beds:4, baths:2, m2:'220', img:'https://images.unsplash.com/photo-1512917774080-9991f1c4c750?w=700&q=80', desc:'Propiedad soñada con amplio parque y pileta climatizada. Barrio residencial tranquilo con seguridad.', status:'active' },
  { id:5, title:'Chalet en barrio residencial', price:'$35.000.000', op:'venta', loc:'Balnearia Sur, Córdoba', beds:3, baths:1, m2:'140', img:'https://images.unsplash.com/photo-1600596542815-ffad4c1539a9?w=700&q=80', desc:'Encantador chalet en barrio tranquilo. Jardín con plantas frutales, galería cubierta y garaje.', status:'active' },
...
function renderDashboard() {
  ...
  document.getElementById('ap-stats-grid').innerHTML = `
    <div class="ap-stat-card">
      ...
      <div class="asc-n">${props.length}</div>
      <div class="asc-l">Total propiedades</div>
    </div>
    <div class="ap-stat-card">
      <div class="asc-top">
        <div></div>
      ...
      <div class="asc-n">${active}</div>
      <div class="asc-l">Propiedades activas</div>
  ...
  document.getElementById('recent-props').innerHTML = recent.length
    ? recent.map(p => `
        <div class="rp-item">
          <img class="rp-img" src="${p.img || FALLBACK_IMG}" onerror="this.src='${FALLBACK_IMG}'" alt="">
          <div style="flex:1">
  ...
  tbl.innerHTML = `
    ...
    ${props.map(p => `
      <div class="prop-table-row">
        <img class="ptr-img" src="${p.img || FALLBACK_IMG}" onerror="this.src='${FALLBACK_IMG}'" alt="">
        <div>
          <div class="ptr-title">${p.title}</div>
          <div class="ptr-sub">${p.loc}</div>
        </div>
...
function showAddForm() {
  ...
  if (lbl) lbl.textContent = 'Seleccionar imagen desde la PC...';
...
function startEdit(id) {
  ...
  if (p.img) {
    ...
    lbl.textContent = 'Imagen cargada (subir nueva para reemplazar)';
  } else {
